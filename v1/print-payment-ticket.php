...
$customer = $employeeClass->getCustomerByIdStaggard($id, $last_id);
$items = $employeeClass->getPawnItem($id,$last_id, 'pawned');

if($_GET['state'] == 2){
    $employeeClass->flushPartial($id, $last_id, 'loan_info_tbl');
}
...
$forfiet_date = date('m/d/Y', strtotime('+10 days', strtotime($due_date)));
if($_GET['state'] == 2){

    if($interest > 0){
        $remaining = $total_loan - $interest;
    }
    else {
        $remaining = $total_loan - $partial_payment;
    }
}
else {
    $remaining = $total_loan - $interest;
...
$total_payback = $total_loan + $interest;
        ...
        <div class="media">
            ...
            <div class="media-body">
                <ul class="breadcrumb">
                    ...
                    <li>Pawn Payment Ticket</li>
                </ul>
                <h4>Pawn Payment Ticket</h4>
            </div>
        </div><!-- media -->
    </div><!-- pageheader -->
                        ...
                        <div class="row">
                            <div id="printablediv">
                                <div class="col-lg-12 print-table">
                                    ...
                                    <?php if ($remaining == 0){ $items = $employeeClass->getPawnItem($id,$last_id, 'Paid'); ?>
                                        <table class="body-table" border="1">
                                            <tr class="border-thick">
                                                <td class="border-thick" rowspan="3"><span class="big-txt">AMOUNT PAID</span><br><small>The Amount You Paid Today</small> <br><span class="big-txt"> <?php echo $amount_paid; ?></span>

                                                </td>
                                                <td class="border-thick" rowspan="3"><span class="big-txt">BALANCE OWED</span><br><small>The remaining balance.</small> <br><span class="big-txt"> <?php echo $remaining; ?>%</span></td>
                                            </tr>
                                            <tr class="border-thick">
                                                <td><h1>PAID IN FULL</h1></td>
                                    ...
                                    <?php }
                                        ...
                                        <table class="body-table" border="1">
                                            <tr class="border-thick">
                                                <td class="border-thick" rowspan="4"><span class="big-txt">ANNUAL <br> PERCENTAGE RATE </span><br><small>The cost of your credit as a yearly rate.</small> <br><span class="big-txt"> <?php echo $apr; ?>%</span></td>
                                                <td class="border-thick" rowspan="4"><span class="big-txt">MONTHLY  <br>PERCENTAGE RATE </span><br><small>The cost of your credit as a monthly rate.</small> <br><span class="big-txt"> <?php echo $rate_first; ?>%</span></td>
                                                <td class="border-thick" rowspan="4"><span class="big-txt">FINANCE <br>   CHARGE </span><br><small>The dollar amount this credit will cost you.</small> <br><span class="big-txt">$<?php echo number_format($interest, 2); ?></span></td>
                                                <td class="border-thick"> <span class="bold">Amount Paid</span> <br> The Amount You Paid Today <br> $<?php echo number_format($amount_paid, 2); ?>
                                                    <?php
                                                    if(isset($_GET['state'])){
                                                        if ($_GET['state'] == 1) {

                                                            $owe = $interest_accured - $partial_payment;
                                                            echo '<br>';
                                                            echo 'You must pay $' . number_format($owe, 2) . ' to extend the contract by ' . $due_date . '';
                                                        }
                                                    }
                                                    ?>
                                            ...
                                            <tr class="border-thick">
                                                <td><span class="bold">Remaining Balance</span><br> The total balance remaining <br>$<?php echo number_format($remaining, 2); ?></td>

                                            </tr>
                                            <tr class="border-thick">
                                                <td><span class="bold">Penalty</span><br> acquired when loan gets past due <br>$<?php echo number_format($penalty, 2); ?></td>

                                            </tr>
                                            <tr class="border-thick">
                                                <td><span class="bold">Total of Payments</span><br> The total you must pay back <br>$<?php echo number_format($total_loan, 2); ?></td>

                                            </tr>
                                            <tr class="border-thin">
                                                <td colspan="2"><span class="bold">Payment Schedule: </span><br> 1 Payment of $<?php echo number_format($total_loan, 2); ?> Due on <?php echo $due_date; ?></td>
                                    ...
                                    <table class="body-table item-display" border="1">
                                        ...
                                        foreach($items as $item){
                                            ...
                                            echo '<td>$' . $loan_amount . '</td>' . PHP_EOL;
